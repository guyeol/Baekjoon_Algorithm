...
int mode(vector<int>arr){
  ...
  for(int i=0;i<arr.size();i++){
    if(m.find(arr[i])==m.end()){
      ...
      m.insert(make_pair(arr[i],m.find(arr[i])->second++));
    }
  }
  int max=0;
  vector<int> _max;
  ...
  for(map<int,int>::iterator it=m.begin();it!=m.end();it++){
    if(it->second == max) {
      _max.push_back(it->first);
    }
  }
  if(_max.size()==1) return _max[0];
  else return _max[1];
}
int range(vector<int>arr){
  if(arr[arr.size()-1]-arr[0]==0) return 0;
  else return arr[arr.size()-1]-arr[0];
}
